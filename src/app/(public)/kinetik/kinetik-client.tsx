...
// Interface dari Supabase (Tanpa Array Konstan lokal)
export interface MataKuliah {
    ...
    prodi: string; // Bisa 'Umum', 'Fisika', 'PendFisika' (varchar Supabase)
    kategori: string | null;
}

// Opsi Penilaian Skala UPI
const GRADES = [
    { label: 'A', value: 4.0 },
    { label: 'A-', value: 3.7 },
    { label: 'B+', value: 3.4 },
...
type SelectedCourse = {
    id: string; // Bisa ID dari Supabase atau custom untuk yang manual
    kode: string;
    nama: string;
    sks: number;
    grade: number | null;
...
export default function KinetikClient({ initialData }: { initialData: MataKuliah[] }) {
    // ==== STATE UTAMA ====
    ...
    const getIpsSemester = (semesterId: number) => {
        ...
        // Tampilkan SKS di box tersebut adalah total SEMUA SKS
    ...
    // Filter DB Berdasarkan Data Array Supabase -> Prodi Aktif -> Search string
    const filteredDBMatkuls = initialData.filter(m => {
        const matchProdi = m.prodi === 'Umum' || m.prodi === selectedProdi;
                    ...
                    <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100/80">
                            ...
                            <p className="text-[11px] text-gray-400 mt-3 text-center">
                                Basis Data (Supabase) memfilter otomatis mata kuliah Anda dari Database Pilihan di *Pop-up*.
                            </p>
                        </div>
                            ...
                            <div className="p-8 text-center relative z-10">
                                ...
                                <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center px-4 bg-black/20 rounded-xl py-4 backdrop-blur-md">
                                    <div className="text-left">
                                        <div className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">SKS Diperhitungkan</div>
                                    ...
                                    <div className="text-right">
                                        <div className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Matkul Dinilai</div>
                            ...
                            {modalTab === 'DB' && (
                                <div className="space-y-4">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            placeholder="Cari nama matkul fisika atau kode UPI..."
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            ...
                                            <div key={m.id} className="bg-white border border-gray-200/60 p-3 rounded-xl hover:border-blue-400 hover:shadow-md transition-all group flex flex-col justify-between cursor-pointer" onClick={() => addMatkulFromDB(m)}>
                                                <div>
                                                    <div className="flex justify-between items-start mb-1">
                                                        ...
                                                        <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wider">Sem {m.semester_rekomendasi}</span>
                                                    </div>
                                        ...
                                        {filteredDBMatkuls.length === 0 && (
                                            <div className="col-span-1 md:col-span-2 text-center py-10 text-gray-500">
                                                Tidak ada mata kuliah yang cocok dari Basis Data Server Supabase. Coba ganti kata kunci.
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <form onSubmit={addManualMatkul} className="space-y-5 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                                    <div className="space-y-4">
                                        <div className="bg-orange-50/50 border border-orange-100 p-3 rounded-lg text-sm text-orange-800 mb-6">
                                            Fitur ini diperuntukkan bagi mata kuliah <strong>Konversi, Lintas Jurusan, atau MBKM</strong> yang SKS-nya tidak ada pada kurikulum himpunan reguler.
                                        </div>
                                        <div>
